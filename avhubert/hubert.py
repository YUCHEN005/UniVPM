...
class AVHubertConfig(FairseqDataclass):
    ...
    num_cluster: int = field(
        default=40, metadata={"help": "number of clusters"}
    )
    bank_capacity: int = field(
        default=800, metadata={"help": "max bank size"}
    )
    init_method: str = field(default='kmeans++', metadata={'help': 'how to initialize cluster centers'})
...
class MIDiscriminator(nn.Module):
    def __init__(self, dim=1024):
        super().__init__()
        self.proj1 = nn.Linear(dim * 2, dim // 2)
        self.proj2 = nn.Linear(dim // 2, dim // 8)
        self.proj3 = nn.Linear(dim // 8, dim // 32)
        self.output_layer = nn.Linear(dim // 32, 1)
    ...
    def forward(self, x_1, x_2):
        ...
        x1 = self.proj1(x)
        x2 = self.proj2(x1)
        x3 = self.output_layer(self.proj3(x2))
        out = self.sigmoid(x3)
        return out

class VPBank(nn.Module):
    def __init__(self, num_cluster=40, bank_capacity=800, init_method='kmeans++'):
        super().__init__()
        self.num_cluster = num_cluster
        self.bank_capacity = bank_capacity
        self.bank = nn.ParameterList([])
        self.cur_batch_centers = []
        self.centers = nn.ParameterList([])
        self.clusters = nn.ParameterList([nn.ParameterList([]) for _ in range(self.num_cluster)])
        self.cluster_sizes = nn.ParameterList([nn.Parameter(torch.Tensor([0])) for _ in range(self.num_cluster)])
        assert init_method in ('kmeans', 'kmeans++'), init_method
        self.init_method = init_method
        self.uninit = nn.Parameter(torch.Tensor([1]))

    def labelme(self, me, centers):
        closest_dist, label = float('inf'), 0
        for lbl, center in enumerate(centers):
            dist = euc_dist(me, center.to(me.device))
            if dist < closest_dist:
                closest_dist, label = dist, lbl
        return closest_dist, label

    def get_init_centers(self):
        bank = deepcopy(self.bank)
        random.shuffle(bank)
        if self.init_method == 'kmeans':
            return bank[:self.num_cluster]
        else:
            centers = nn.ParameterList([bank[0]])
            dists = [0 for _ in range(len(bank))]
            while len(centers) < self.num_cluster:
                for i, p in enumerate(bank):
                    dist, _ = self.labelme(p, centers)
                    dists[i] = math.pow(dist, 2)
                cum_dist = sum(dists) * random.random()
                for i, dist in enumerate(dists):
                    cum_dist -= dist
                    if cum_dist > 0:
                        continue
                    centers.append(bank[i])
                    break
            del bank
            return centers
    ...
    def random_prune(self):
        bank = deepcopy(self.bank)
        random.shuffle(bank)
        bank = bank[:self.bank_capacity]
        bank.extend(self.centers)
        self.bank = list(set(bank))
        del bank

    def renew_centers(self):
        for i, cluster in enumerate(self.clusters):
            num_sample = len(cluster)
            device = self.bank[0].device

            if num_sample == 0:
                self.cluster_sizes[i] = nn.Parameter(torch.Tensor([1]))
                self.cluster_sizes[i].requires_grad = False
                self.clusters[i].append(self.centers[i].to(device))
                self.bank.append(self.centers[i].to(device))
            else:
                self.cluster_sizes[i] = nn.Parameter(torch.Tensor([num_sample]))
                self.cluster_sizes[i].requires_grad = False
                center = nn.Parameter(torch.zeros_like(cluster[0])).to(device)
                for c in cluster:
                    center = center + c.to(device)
                self.centers[i] = center / num_sample

    def undersample(self, id, avg_size):
        center = self.centers[id]
        this_cluster = self.clusters[id]
        dists = []
        for i, sample in enumerate(this_cluster):
            dists.append(euc_dist(center, sample.to(center.device)))
        topk_ids = heapq.nsmallest(avg_size, range(len(dists)), dists.__getitem__)
        self.clusters[id] = nn.ParameterList([this_cluster[i] for i in topk_ids])
        self.cluster_sizes[id] = nn.Parameter(torch.Tensor([avg_size]))

    def upsample(self, id):
        center = self.centers[id]
        closest_dist, closest_id = float('inf'), 0
        this_cluster = self.clusters[id]
        for i, sample in enumerate(this_cluster):
            dist = euc_dist(sample.to(center.device), center)
            if dist < closest_dist:
                closest_dist, closest_id = dist, i
        alpha = random.random()
        new_sample = this_cluster[closest_id] * alpha + center * (1 - alpha)
        self.clusters[id].append(new_sample)
        self.cluster_sizes[id] = self.cluster_sizes[id] + torch.Tensor([1])

    def balanced_update(self, batch):  # [(D), (D), ...]
        cur_batch_size = len(batch)
        for sample in batch:
            self.bank.append(nn.Parameter(sample))
        # initialize the cluster centers
        if self.uninit[0] == 1:
            if len(self.bank) < self.bank_capacity:
                return
            else:
                logging.info(f"len(self.bank): {len(self.bank)} ==> init centers!")
                self.centers = self.get_init_centers()
                self.uninit[0] = torch.tensor(0)

        # reallocate each sample to the nearest center
        bank_size = len(self.bank)
        self.clusters = nn.ParameterList([nn.ParameterList([]) for _ in range(self.num_cluster)])
        self.cur_batch_centers = []
        for i, sample in enumerate(self.bank):
            _, label = self.labelme(sample, self.centers)
            self.clusters[label].append(sample)
            with TemporaryGrad():
                if i >= bank_size - cur_batch_size:
                    self.cur_batch_centers.append(self.centers[label])

        # renew the cluster centers
        self.renew_centers()

        # re-sample (undersample or upsample)
        avg_size = min(int(sum(self.cluster_sizes) // self.num_cluster), self.bank_capacity // self.num_cluster)
        self.bank = nn.ParameterList([])
        for i, cluster_size in enumerate(self.cluster_sizes):
            if cluster_size > avg_size:         # majority cluster
                self.undersample(i, avg_size)
                self.bank.extend(self.clusters[i])
            elif cluster_size < avg_size:       # minority cluster
                self.upsample(i)
                self.bank.extend(self.clusters[i])
            else:
                self.bank.extend(self.clusters[i])

    def get_centers_var(self):
        mean_center = sum(self.centers) / self.num_cluster
        var = sum([(center - mean_center).float().pow(2).sum() for center in self.centers])
        return var
...
class AVHubertModel(BaseFairseqModel):
    def __init__(
        ...
        # viseme/phoneme bank
        self.num_cluster = cfg.num_cluster
        self.bank_capacity = cfg.bank_capacity
        self.init_method = cfg.init_method

        self.vise_bank = VPBank(num_cluster=self.num_cluster, bank_capacity=self.bank_capacity,
                                init_method=self.init_method)
        self.phone_bank = VPBank(num_cluster=self.num_cluster, bank_capacity=self.bank_capacity,
                                 init_method=self.init_method)

        # discriminator
        self.mi_discriminator = MIDiscriminator(self.encoder_embed_dim)
    ...
    def extract_finetune(self, source, padding_mask=None, mask=False, ret_conv=False, output_layer=None):
        ...
        features_video = features_video.transpose(1, 2)     # [B, T, D]
        features_audio = features_audio.transpose(1, 2)     # [B, T, D]

        # update viseme/phoneme bank
        with torch.no_grad():
            if random.random() < 0.1:                   # update banks every 10 epochs
                vframes = features_video.flatten(0, 1)  # [B*T, D]
                pframes = features_audio.flatten(0, 1)

                num_frame = vframes.size(0)
                select_vframes, select_pframes = [], []

                if num_frame > self.vise_bank.bank_capacity // 2:
                    select_ids = random.sample(range(0, num_frame), self.vise_bank.bank_capacity // 2)
                    select_vframes = [vframes[id] for id in select_ids]
                    select_pframes = [pframes[id] for id in select_ids]
                else:
                    select_vframes = [vframe for vframe in vframes]
                    select_pframes = [pframe for pframe in pframes]

                # update viseme/phoneme bank
                self.vise_bank.balanced_update(select_vframes)
                self.phone_bank.balanced_update(select_pframes)

        with TemporaryGrad():
            vcenters_var_penalty, pcenters_var_penalty = torch.Tensor([0]).to(features_video.device), torch.Tensor([0]).to(features_video.device)
            cur_batch_vcenters, cur_batch_pcenters = None, None
            if self.phone_bank.uninit[0] == 0:
                # enlarge center variance -> make cluster centers scattered
                vcenters_var_penalty = self.vise_bank.get_centers_var()
                pcenters_var_penalty = self.phone_bank.get_centers_var()

                # v-p mapping
                cur_batch_vcenters = torch.stack([c for c in self.vise_bank.cur_batch_centers], dim=0)     # [N, D]
                cur_batch_pcenters = torch.stack([c for c in self.phone_bank.cur_batch_centers], dim=0)    # [N, D]

            aux_outputs['var_penalty'] = vcenters_var_penalty + pcenters_var_penalty
            aux_outputs['centers'] = [cur_batch_vcenters, cur_batch_pcenters]

            ### modality transfer
            v2p_transfer = None
            if self.phone_bank.uninit[0] == 0:
                B, T, D = features_video.size()
                vfeats_ = features_video.view(-1, D)                        # [B*T, D]
                phone_centers = torch.stack([c for c in self.phone_bank.centers], dim=0).to(vfeats_.device)      # [40, D]

                sim_type = 'cosine'
                if sim_type == 'dot':
                    logits = torch.matmul(vfeats_, phone_centers.T).view(B, T, -1)      # [B, T, 40]
                elif sim_type == 'cosine':
                    nom = (vfeats_.unsqueeze(dim=1) * phone_centers.unsqueeze(dim=0)).sum(dim=-1)       # [B*T, 40]
                    denom = (vfeats_ ** 2).sum(dim=-1).sqrt().unsqueeze(dim=1) * (phone_centers ** 2).sum(dim=-1).sqrt().unsqueeze(dim=0)   # [B*T, 40]
                    logits = (nom / denom.clamp(min=1e-6)).view(B, T, -1)               # [B, T, 40]
                else:
                    raise NotImplementedError

                v2p_transfer = torch.matmul(
                    torch.softmax(logits / self.logit_temp, dim=-1), phone_centers)     # [B, T, D]

            aux_outputs['v2p_transfer'] = v2p_transfer
            aux_outputs['feats'] = [features_video.clone(), features_audio.clone()]

        # concat
        if v2p_transfer is not None:
            features_audio = features_audio + v2p_transfer
        ...
        return x, padding_mask, aux_outputs

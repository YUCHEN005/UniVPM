...
class FairseqTask(object):
    ...
    def train_step_first(
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                loss1, loss2, sample_size, logging_output = criterion(model, sample)
        if ignore_grad:
            loss1 *= 0
            loss2 *= 0
        with torch.autograd.profiler.record_function("backward"):
            optimizer.backward(loss1, retain_graph=True)
        return loss1, loss2, sample_size, logging_output

    def forward_backward_second(
        self, loss, model, optimizer, update_num
    ):
        """
        Do forward and backward, and return the loss as computed by *criterion*
        for the given *model* and *sample*.
        ...
        with torch.autograd.profiler.record_function("backward"):
            optimizer.backward(loss, retain_graph=False)

    def train_step_second(
        self, sample, model, criterion, optimizer, update_num, ignore_grad=False
    ):
        """
        Do forward and backward, and return the loss as computed by *criterion*
        ...
        if ignore_grad:
            ...
            loss2 *= 0
        with torch.autograd.profiler.record_function("backward"):
            optimizer.backward(loss2, retain_graph=False)
        return loss1, loss2, sample_size, logging_output
